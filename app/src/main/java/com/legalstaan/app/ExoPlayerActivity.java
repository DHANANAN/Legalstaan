...
/**
 * ExoPlayer-based player wrapping Google Drive direct stream.
 * For shared Drive videos, the public download URL streams as MP4 — ExoPlayer
 * can play it natively. If the file is restricted or playback fails, we fall
 * back to {@link VideoActivity} which uses the embedded Drive WebView preview
 * (cookie-aware, slower but always works).
 */
...
public class ExoPlayerActivity extends AppCompatActivity {
    ...
    // Spoof a real desktop browser so Drive serves the video without the WebView interstitial.
    ...
    private boolean    fellBackToWebView = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Edge-to-edge — give the player every available pixel
    ...
    private void initPlayer(String fileId, String title) {
        ...
        // Direct download URL — Drive returns a 302 to the actual mp4 stream on googleusercontent.com.
        ...
        player.addListener(new Player.Listener() {
            ...
            public void onPlayerError(PlaybackException error) {
                // Restricted Drive file or HTML interstitial — graceful fallback to the
                // existing WebView player so the user still gets to watch.
                if (fellBackToWebView) return;
                fellBackToWebView = true;
                ui.post(() -> {
                    Toast.makeText(ExoPlayerActivity.this,
                            "Switching to compatibility player…", Toast.LENGTH_SHORT).show();
